...
Функция СоздатьЗадачу(Структ)
	...
	ДокЗадача.ИдПисьма = СокрЛП(Структ.ИдентификаторСообщения);
	ДокЗадача.Состояние = Справочники.СостоянияЗадач.Создана;
	ДокЗадача.НаименованиеЗадачи = Структ.Тема;
	ДокЗадача.ОписаниеЗадачи = Новый ХранилищеЗначения(Структ.Текст);
	ДокЗадача.ПостановщикЗадачи = Справочники.Пользователи.Почта;
	ДокЗадача.Заказчик = НайтиКонтакт(Структ.Отправитель, "email");
	ДокЗадача.ПроектГруппа = НайтиГруппуКонтакта(ДокЗадача.Заказчик);
	
	Попытка
		ДокЗадача.Записать();
		Рет = "Задача " + ДокЗадача + " создана!";
	Исключение
